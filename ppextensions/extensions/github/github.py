...
class PrivateGitHandler(IPythonHandler):
    ...
    def __init__(self):
        ...
        self.GITHUB_API_PREFIX = "https://api.github.com"
        self.GITHUB_TOKEN = os.getenv('githubtoken', "")
        self.NOTEBOOK_STARTUP_PATH = os.getcwd() + "/"
        self.LOCAL_REPO_FOLDER = "Sharing"
        self.LOCAL_REPO_PREFIX = self.NOTEBOOK_STARTUP_PATH + self.LOCAL_REPO_FOLDER
    ...
    def git_clone(self, local_repo_path, repo_url):
        try:
            repo_instance = Repo(local_repo_path)
        except exc.NoSuchPathError:
            o = urlparse(repo_url)
            repo_url_with_token = o.scheme + "://" + self.GITHUB_TOKEN + "@" + o.hostname + o.path
    ...
    def get_repo(self, file_path):
        ...
        if parts[0] == self.LOCAL_REPO_FOLDER:
            repo_name = parts[1] + "/" + parts[2]
            branch = requests.get(self.GITHUB_API_PREFIX + '/repos/' + repo_name + '/branches', headers=headers)
            ...
            repo = requests.get(self.GITHUB_API_PREFIX + '/user/repos', headers=headers, params=params).json()
            for rp in repo:
...
class PrivateGitPushHandler(PrivateGitHandler):
    ...
    def post(self):
        ...
        repo_url = self.GITHUB_URL_PREFIX + repo_name + ".git"
        local_repo_path = self.LOCAL_REPO_PREFIX + "/" + repo_name
        if not file_path.startswith(self.LOCAL_REPO_FOLDER):
            local_repo_file_path = local_repo_path + "/" + file_path
        else:
            local_repo_file_path = self.NOTEBOOK_STARTUP_PATH + file_path
        repo_instance = self.git_clone(local_repo_path, repo_url)
        try:
            self.git_commit(self.NOTEBOOK_STARTUP_PATH + file_path, local_repo_file_path, file_name, repo_instance,
                            commit_message)
...
class PrivateGitPullHandler(PrivateGitHandler):
    ...
    def post(self):
        ...
        local_repo_path = self.LOCAL_REPO_PREFIX + repo
        try:
            repo_instance = self.git_clone(local_repo_path, repo_url)
            git_instance = repo_instance.git
...
class PrivateGitCommitHandler(PrivateGitHandler):
    ...
    def post(self):
        ...
        local_repo_path = self.NOTEBOOK_STARTUP_PATH + repo_name
        try:
            repo_instance = Repo(local_repo_path)
