...
#We set LOCAL REPO PREFIX equal to USER_HOME_PATH_PREFIX to encourage users to edit inside the local repo
LOCAL_REPO_PREFIX = "/home/" + getuser() + "/Private Sharing/"
...
class PrivateGitHandler(IPythonHandler):
    ...
    def gitcommit(from_path, to_path, repo_instance, commit_message):
        #we still do the copy since currently not all notebooks in users' home folder are in local repo.
        try:
    ...
    def gitcommitsinglefile(repo_name, file_name, commit_message):
        local_repo_path = USER_HOME_PATH_PREFIX + repo_name 
        try:
            repo_instance = Repo(local_repo_path) 
...
class PrivateGitPushHandler(PrivateGitHandler):
    def post(self):
        ...
        file_path = self.get_argument("filepath") #somehow it is able to get the correct file even though it is not the full path
        ...
        message2 = "Your notebook has been moved to local github repo: Private Sharing/" + repo_name if file_path != local_repo_file_path and message1 == "" else ""
